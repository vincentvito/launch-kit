...
export default function LoginClient({ googleEnabled }: { googleEnabled: boolean }) {
  ...
  const handleEmailSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.error) {
        setError(result.error.message || t('emailError'))
        return
      }
        ...
        <form onSubmit={handleEmailSubmit} className="space-y-4">
          {mode === 'signUp' ? (
            <label className="block space-y-1.5">
              <span className="text-sm font-medium text-gray-700 dark:text-zinc-200">{t('nameLabel')}</span>
              <input
                value={name}
          ...
          <label className="block space-y-1.5">
            <span className="text-sm font-medium text-gray-700 dark:text-zinc-200">{t('emailLabel')}</span>
            <input
              value={email}
          ...
          <label className="block space-y-1.5">
            <span className="text-sm font-medium text-gray-700 dark:text-zinc-200">{t('passwordLabel')}</span>
            <input
              value={password}
          ...
          <Button type="submit" size="lg" className="w-full" disabled={submitting}>
            {submitting ? t('submitting') : mode === 'signIn' ? t('emailSignIn') : t('emailSignUp')}
          </Button>
        </form>
